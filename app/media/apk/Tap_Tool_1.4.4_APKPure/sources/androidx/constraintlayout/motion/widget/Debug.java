...
public class Debug {
    ...
    public static void printStack(String msg, int n) {
        ...
        for (int i = 1; i <= min; i++) {
            ...
            printStream.println(msg + str + (".(" + stackTrace[i].getFileName() + ":" + stackTrace[i].getLineNumber() + ") ") + str);
        }
    }
    ...
    public static String getState(MotionLayout layout, int stateId, int len) {
        ...
        if (len != -1) {
            ...
            if (resourceEntryName.length() <= len || (length = resourceEntryName.replaceAll("[^_]", RequestConfiguration.MAX_AD_CONTENT_RATING_UNSPECIFIED).length()) <= 0) {
