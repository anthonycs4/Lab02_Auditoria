...
public final class zzfxh extends OutputStream {
    ...
    private final ArrayList<zzfxj> zzc = new ArrayList<>();
    ...
    private final void zzc(int i) {
        this.zzc.add(new zzfxg(this.zze));
    ...
    public final synchronized zzfxj zza() {
        int i = this.zzf;
        byte[] bArr = this.zze;
        int length = bArr.length;
        if (i >= length) {
            this.zzc.add(new zzfxg(bArr));
            this.zze = zza;
        } else if (i > 0) {
            byte[] bArr2 = new byte[i];
            System.arraycopy(bArr, 0, bArr2, 0, Math.min(length, i));
            this.zzc.add(new zzfxg(bArr2));
        }
        this.zzd += this.zzf;
        this.zzf = 0;
        return zzfxj.zzx(this.zzc);
    }
